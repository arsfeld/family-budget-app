...
export async function inviteFamilyMember({ 
  ...
  try {
    ...
    console.log(`Invitation would be sent to ${email} to join family ${familyId}`)
...
export async function createCategory({ name, icon, color }: { name: string; icon: string; color: string }) {
  const session = await auth()
...
export async function updateCategory({ id, name, icon, color }: { id: string; name: string; icon: string; color: string }) {
  const session = await auth()
  ...
  try {
    ...
    await db.$transaction(async (tx) => {
      ...
      for (const oldCat of currentCategories) {
        ...
        let matchedNewCat = allDefaultCategories.find(nc => 
        ...
        if (!matchedNewCat) {
          ...
          for (const [key, defaultName] of Object.entries(mappings)) {
            if (lowerName.includes(key)) {
              matchedNewCat = allDefaultCategories.find(nc => nc.name === defaultName)
              break
            }
          }
        }
        ...
        if (!matchedNewCat) {
          matchedNewCat = allDefaultCategories.find(nc => nc.name === 'Other')!
        }
      ...
      for (const [oldCatId, newCatId] of categoryMapping) {
...
export async function getResetCategoriesPreview() {
  ...
  try {
    ...
    const preview = currentCategories.map(cat => {
      ...
      let matchedDefault = DEFAULT_CATEGORIES.find(dc => 
      ...
      if (!matchedDefault) {
        ...
        if (lowerName.includes('hous') || lowerName.includes('home') || lowerName.includes('mortgage') || lowerName.includes('rent')) {
          matchedDefault = DEFAULT_CATEGORIES.find(dc => dc.name === 'Housing')
        } else if (lowerName.includes('transport') || lowerName.includes('car') || lowerName.includes('vehicle')) {
          matchedDefault = DEFAULT_CATEGORIES.find(dc => dc.name === 'Transportation')
        } else if (lowerName.includes('child') || lowerName.includes('daycare') || lowerName.includes('kids')) {
          matchedDefault = DEFAULT_CATEGORIES.find(dc => dc.name === 'Childcare')
        } else {
    ...
    return { 
      success: true, 
      preview,
      willAddCategories: DEFAULT_CATEGORIES.filter(dc => 
        !currentCategories.some(cc => cc.name.toLowerCase() === dc.name.toLowerCase())
      )
    }
  } catch (error) {
